...
- **三种模式**：练习模式、限时挑战（60 秒内尽可能多答，答错会停留并显示解析）、错题重刷
...
- **外观**：跟随系统 / 浅色 / 深色三档主题，背景为多层柔和光晕，深色模式适合夜间练习
...
| 限时挑战 | 总时长 60 秒，答对 0.4 秒后自动进入下一题；答错则停留 2 秒展示解析，按 `Enter` 可提前跳过，结束后结算答对题数 |
...
└── components/
    ├── SetupView.jsx        # 首页：出题范围、限时、外观、成绩概览
