...
- **三种模式**：练习模式、限时挑战（60 秒内尽可能多答）、错题重刷
- **每题限时**：不限时 / 20 秒 / 30 秒，超时按答错计并同样进入错题本
- **双条解析**：答错时同时给出正确项与所选错项的解析，一眼看清「它到底属于谁」
- **错题本**：答错或超时自动收录，重刷答对后自动移出，按错误次数排序
- **成绩统计**：累计题量、正确率、最佳连对、挑战纪录，以及按上级行政区拆分的正确率
- **键盘操作**：`A`–`D` 或 `1`–`4` 作答，`Enter` 进入下一题
- **零依赖后端**：所有数据存在浏览器 localStorage，刷新不丢，清除浏览器数据才会清空
...
| 两者混合 | 407 条 | 按题库规模混出，地级行政区题目占比更高 |

| 模式 | 规则 |
| --- | --- |
| 练习模式 | 不限题数，每题可单独设置限时，答完可看解析 |
| 限时挑战 | 总时长 60 秒，答完自动进入下一题，结束后结算答对题数 |
...
│   └── storage.js           # localStorage 读写与统计/错题本操作
└── components/
    ├── SetupView.jsx        # 首页：出题范围、限时、成绩概览
    ├── QuizView.jsx         # 答题页：题干、选项、计时、反馈与解析
    ├── ResultView.jsx       # 单次结算
    ├── WrongBookView.jsx    # 错题本
    └── StatsView.jsx        # 数据页：累计指标与按上级行政区拆分
docs/
...
| Key | 内容 |
| --- | --- |
| `xiaoce.stats.v1` | 累计题量、正确数、最佳连对、挑战纪录、按上级行政区的分布 |
| `xiaoce.wrong.v1` | 错题条目：行政区、上级行政区、错误次数、最近错误时间 |
| `xiaoce.settings.v1` | 出题范围、每题限时 |

在「数据」页可以清零统计，在「错题本」页可以清空错题，两者互不影响。
